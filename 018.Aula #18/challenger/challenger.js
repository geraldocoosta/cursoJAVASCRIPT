(function(){
	...
	*/
	...
	function cleanCPF(cpfParam){
		return cpfParam.replace( /\D/g,'').match(/[\d]{11}/)[0];
	};
	var cpfs = ["049-214 3421-1","210.458.522-05","735 500 794 - 22","101.123-131x32"];

	cpfs.forEach(function( cpf ){ console.log(cleanCPF(cpf)) });
	...
	*/
	...
	function correctCPF(cpfLimpo){
		return cpfLimpo.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/,'$1.$2.$3-$4');
	}

	cpfs.forEach(function( cpf ){ console.log(correctCPF(cleanCPF(cpf))) });
	...
	*/
	...
	var frase2 = "<div><section><blockquote>Texto <img /></blockquote></section></div>";
	var marcacoes = frase2.match(/\<[\w]+\>/g);
	...
	*/
	...
	var frase3 = "<div><ul><li></li><li></li><li><span></span></li></ul></div>"
	marcacoes = frase3.match(/\<([\w]+)\>\<\/([\w]+)\>/g);
	console.log(marcacoes);
	...
	*/
	...
	function captura(regex, cap1, cap2){
		return '<'+cap1 + '> O texto dentro da tag ' + cap1 + ' é "' + cap2 +'" '  ;
	}
	var frase4 = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
	marcacoes = frase4.replace(/\<([\w]+)\>([^\<\>\\]+)/g,captura);
	console.log(marcacoes);
})();